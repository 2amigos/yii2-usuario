...
use Da\User\Helper\Gravatar;
use Da\User\Query\ProfileQuery;
use Da\User\Traits\ContainerAwareTrait;
use Da\User\Traits\ModuleAwareTrait;
use Da\User\Validator\TimeZoneValidator;
use Da\Usuario\Base\Contracts\UsuarioProfile;
use DateTime;
use DateTimeZone;
use Exception;
use Yii;
 ...
 * @property User $user
 */
class Profile extends ActiveRecord implements Profile
{
    ...
    public function beforeSave($insert)
    {
        if ($this->isAttributeChanged('gravatar_email')) {
            $this->setAttribute(
                'gravatar_id',
                $this->make(Gravatar::class)->buildId(trim($this->getAttribute('gravatar_email')))
            );
        }
    ...
    {
        return $this->make(Gravatar::class)->getUrl($this->gravatar_id, $size);
    }
